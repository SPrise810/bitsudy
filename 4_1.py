...
#         print(value)
 ##
 ##要创建数字列表，可使用 函数list() 将range()的结果直接转换成列表，如果将range()作为list()的参数， 输出将是一个数字列表
...
# shuzi2=shuzi1[:]
# print(shuzi2)
#如果我们直接使用复制，而不是通过切片将 副本 赋值给新的变量，那么我们如果要修改新的列表中的内容，那么原列表也会被操作
##
## 元组
##列表非常适合存储 在程序运行期间可能变化的数据集。列表可以修改，但有时候需要一些不能被修改的元素，那么元组可以满足这种需求。
##定义元组  顾名思义，括号就用圆括号
##需要说明，元组是由逗号标识的，我们如果想要定义一个只包含一个元素的元组，必须在后面加上，  例如  juxing=(3,)
# juxing=(200,50)
# print(juxing[0])
# print(juxing[1])
