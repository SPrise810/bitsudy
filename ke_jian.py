...
# #函数主体 s=x*x
# return s 返回变量
# 返回值不是必须的，如果没有return语句，则
# Python默认返回值None
#
# #几种参数设定方法   – 顺序传入   – 关键词   – 默认参数   – 不定长参数 （注意不同于C++等语言
# #                              square(x=1)
# 如果传入的第一个参数是用关键词传入的，那么后面每个参数都需要是关键词传入，否则会出现语法错误
# #默认参数必须放到参数列表的末位  square(1,2,x=3)
#
# #函数定义时，python使用*args的形式，解决不能确定需要传入参数个数的情况，*args可以接收打包为元组形式的多个形参
# #def fun(x,*args)
#  for i in ages:
#   print(x+i)
# #fun(1,1,2,3)
...
# #
#  函数调用时，
# *可以“解压”待传递到函数中的 元组、列表、
# 集合、字符串等类型，并按位置传递到函数入口参数中
# 聚不类定分长析 参数*args
# def sum1(a,b,c,d,e):
# return a+b+c+d+e
# tup1 = (1,2,3,4,5)
